...
(() => {
  ...
  // src/utils/animations/barba/barbaLogoRotate.ts
  init_live_reload();
  var barbaLogoRotate = () => {
  ...
  // src/utils/animations/barba/barbaResetVideo.ts
  init_live_reload();
  ...
  // src/utils/animations/carousel.ts
  init_live_reload();
  ...
  function duplicateUntilWideEnough(root, wrapper) {
    ...
    while (wrapper.scrollWidth < targetWidth && safety < 50) {
      originalSlides.forEach((slide2) => {
        wrapper.appendChild(slide2.cloneNode(true));
      });
      wrapper.offsetWidth;
      safety += 1;
    }
  }
  function computeSpeed(wrapper) {
    const totalWidth = wrapper.scrollWidth;
    return Math.max(1e3, totalWidth / PIXELS_PER_SECOND * 1e3);
  }
  function initLoopSwiperForRoot(root) {
    ...
    swiper.on("resize", (s) => {
      ...
      s.params.speed = computeSpeed(wrapper);
      s.update();
    });
  }
  function initLoopWordSwiper(options = { rootSelector: ".swiper.is-loop-word" }) {
    ...
    swipers.forEach((el) => initLoopSwiperForRoot(el));
  }
  ...
  // src/utils/animations/footerAnimation.ts
  init_live_reload();
  ...
  // src/utils/animations/heartBeat.ts
  init_live_reload();
  ...
  // src/utils/animations/howSlider.ts
  init_live_reload();
  ...
  // src/utils/animations/networkGradient.ts
  init_live_reload();
  ...
  // src/utils/animations/OtherProjectsSlider.ts
  init_live_reload();
  ...
  // src/utils/animations/rainbow.ts
  init_live_reload();
  ...
  // src/utils/animations/whereProjectsScroll.ts
  init_live_reload();
  ...
  function initWhereProjectsScroll() {
    const heading = document.querySelector(".where_projects_heading");
    if (!heading) {
      return;
    }
  ...
  // src/utils/animations/whoSlider.ts
  init_live_reload();
  ...
  // src/utils/animations/whyAnimations.ts
  init_live_reload();
  ...
  // src/utils/animations/whyLetterScroll.ts
  init_live_reload();
  function wrapWordsInSpan(element) {
    const text = element.textContent || "";
    element.innerHTML = text.split(" ").map((word) => `<span class="word">${word}</span>`).join(" ");
  }
  function initWhyLetterScroll() {
    const pinHeight = document.querySelector(".section_home_why");
    if (!pinHeight) {
      console.error("\u274C Section .section_home_why not found");
      return;
    }
    const container = pinHeight.querySelector(".home_why_component");
    const paragraph = pinHeight.querySelector(".home_why_heading");
    if (!container) {
    ...
    ScrollTrigger2.create({
      markers: false,
      trigger: pinHeight,
      // We listen to .section_home_why position
      start: "top top",
      end: "bottom bottom",
      pin: container
      // Progresses with the scroll
    });
    const lines = [[]];
    let lineIndex = 0;
    ...
    lines.forEach((lineWords) => {
      gsapWithCSS.to(lineWords, {
        x: 0,
        // Animate the 'x' property to 0
        stagger: 0.2,
        // Stagger the animation of each element by 0.2
        ease: "power1.inOut",
        scrollTrigger: {
          trigger: pinHeight,
          ...
          scrub: true
          // Scrub the animation based on scroll position
        }
      });
    });
  }

  // src/utils/animations/worksMouse.ts
  init_live_reload();
  function worksMouse() {
    ...
    const mouseTextWrapper = mouseComponent.querySelector(".works-mouse_text-wrapper");
    if (mouseTextWrapper) {
    ...
    transitions: [
      {
        ...
        enter(data) {
          gsapWithCSS.from(data.next.container, {
            y: -16 * 2,
            opacity: 0,
            duration: 0.5,
            ease: "power2.out",
